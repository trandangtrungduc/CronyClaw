...
import { Stack, Button, Text, createListCollection } from "@chakra-ui/react";
...
function General({ onSave, onCancel }: GeneralProps): JSX.Element {
  ...
  return (
    <Stack {...settingStyles.common.container}>
      ...
      <InputField
        label={t("settings.general.openclawBridgeUrl")}
        value={settings.openclawBridgeUrl}
        onChange={(value) => handleSettingChange("openclawBridgeUrl", value)}
        placeholder="ws://127.0.0.1:18789"
      />

      <InputField
        label={t("settings.general.openclawBridgeToken")}
        value={settings.openclawBridgeToken}
        onChange={(value) => handleSettingChange("openclawBridgeToken", value)}
      ...
      <Button
        colorPalette="teal"
        onClick={() => {
          void checkOpenClawBridge();
        }}
      >
        <Text>{t("settings.general.openclawBridgeCheck")}</Text>
      </Button>
      <Text color={bridgeStatus.connected ? "green.300" : "orange.300"}>
        {bridgeStatus.connected
          ? t("settings.general.openclawBridgeConnected")
          : t("settings.general.openclawBridgeNotConnected")}
      </Text>
      {!bridgeStatus.connected && bridgeStatus.lastError && (
        <Text color="orange.200">{bridgeStatus.lastError}</Text>
      )}
      {!bridgeStatus.connected && bridgeStatus.hint && (
        <Text color="yellow.200">{bridgeStatus.hint}</Text>
      )}
