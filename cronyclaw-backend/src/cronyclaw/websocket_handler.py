...
class WebSocketHandler:
    ...
    def _init_message_handlers(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            "cronyclaw-notify": self._handle_cronyclaw_notify,
        }
    ...
    async def _init_service_context(
        ...
        await session_service_context.load_cache(
            ...
            mcp_server_registry=self.default_context_cache.mcp_server_registry,
    ...
    async def broadcast_external_notification(
        self, text: str, claw_name: Optional[str] = "OpenClaw"
    ) -> int:
        text = (text or "").strip()
        if not text:
            return 0

        user_input = text if not claw_name else f"[{claw_name}] {text}"
        trigger_payload: WSMessage = {
            "type": "text-input",
            "text": user_input,
            "metadata": {
        ...
        for client_uid, websocket in list(self.client_connections.items()):
            try:
                await self._handle_conversation_trigger(
                    websocket, client_uid, trigger_payload
                )
                delivered_clients += 1
            except Exception as e:
                logger.error(
                    f"Failed to deliver external notification to {client_uid}: {e}"
                )
    ...
    async def _handle_cronyclaw_notify(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        await self.broadcast_external_notification(
            text=data.get("text", ""),
            claw_name=data.get("claw_name", "OpenClaw"),
        )