...
class BasicMemoryAgent(AgentInterface):
    ...
    def __init__(
        ...
        self._openclaw_command_mode = False
        self._openclaw_command_task = ""
        self._openclaw_skill_prompt = ""
        self._openclaw_skill_path = Path(
            "src/cronyclaw/skills/openclaw-cli/SKILL.md"
        )
    ...
    def _parse_openclaw_command(self, text_prompt: str) -> tuple[bool, str]:
        stripped = text_prompt.strip()
        if not stripped:
            return False, text_prompt
        if not stripped.lower().startswith("/openclaw"):
            return False, text_prompt
        task = stripped[len("/openclaw") :].strip()
        return True, task

    def _load_openclaw_skill_prompt(self) -> str:
        try:
            return self._openclaw_skill_path.read_text(encoding="utf-8").strip()
        except Exception as e:
            logger.warning(f"Failed to load OpenClaw skill prompt: {e}")
            return ""

    def _build_runtime_system_prompt(self, prompt_mode: bool) -> str:
        system_prompt = self._system
        if prompt_mode and self._mcp_prompt_string:
            system_prompt = f"{system_prompt}\n\n{self._mcp_prompt_string}"
        elif prompt_mode and not self._mcp_prompt_string:
            logger.warning("Prompt mode active but mcp_prompt_string is empty!")

        if self._openclaw_command_mode:
            instruction = (
                "Command mode active: user invoked /openclaw.\n"
                "You must read and follow the OpenClaw CLI skill guidance below.\n"
                "If request is executable and parameters are sufficient, call tool `exec` using shell `bash`.\n"
                "If required parameters are missing, ask one concise clarification question."
            )
            if self._openclaw_skill_prompt:
                system_prompt = (
                    f"{system_prompt}\n\n{instruction}\n\nOpenClaw CLI Skill:\n{self._openclaw_skill_prompt}"
                )
            else:
    ...
    def _to_messages(self, input_data: BatchInput) -> List[Dict[str, Any]]:
        ...
        is_openclaw_mode, openclaw_task = self._parse_openclaw_command(text_prompt)
        self._openclaw_command_mode = is_openclaw_mode
        self._openclaw_command_task = openclaw_task
        self._openclaw_skill_prompt = (
            self._load_openclaw_skill_prompt() if is_openclaw_mode else ""
        )
        if is_openclaw_mode:
            task_text = (
                openclaw_task
                if openclaw_task
                else "Interpret the OpenClaw CLI task from user context."
            )
            text_prompt = (
                "User invoked /openclaw command mode.\n"
    ...
    async def _openai_tool_interaction_loop(
        ...
        while True:
            if self.prompt_mode_flag:
                current_system_prompt = self._build_runtime_system_prompt(
                    prompt_mode=True
                )
                tools_for_api = None
            else:
        ...
        async def chat_with_memory(
            ...
            if self._use_mcpp and tool_mode == "Claude":
                ...
                token_stream = self._llm.chat_completion(
                    messages,
                    self._build_runtime_system_prompt(prompt_mode=False),
                )
                complete_response = ""
